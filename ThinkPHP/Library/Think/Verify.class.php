...
class Verify {
    ...
    public function __set($name,$value){
        //isset($this->config['setCode'])
        if(isset($this->config[$name])) {
            //$this->config['setCode'] = '123456789';
    ...
    public function entry($id = '') {
        ...
        if($this->useZh){ // 中文验证码
            ...
            for ($i = 0; $i<$this->length; $i++) {
                ...
                imagettftext($this->_image, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize*1.6, ``, $this->fontttf, $code[$i]);
            }
        }
    ...
    public function entry1($id = '') {
        ...
        for ($i = 0; $i<$this->length; $i++) {
            $code[$i] = $this->codeSet[mt_rand(0, strlen($this->codeSet)-1)];
            $codeNX = $this->fontSize * $i + 10;
            imagettftext(
                $this->_image, 
                $this->fontSize,   //字体大小 配置项中设置的15
                0,      //字体倾斜角度 
                $codeNX, 
                $this->fontSize*1.6, 
                $this->_color, 
